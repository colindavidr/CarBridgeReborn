...
// appVC.view state for the first ~2s (per bid) so a WORKING app (TrollStore/Dropbox) can be diffed
// against an abrupt one, and fires the zoom-in of the LIVE app out of the icon once content is
// present (0.8s fallback). It never stops - when idle it ticks slowly and writes nothing.
static void cbrAnimHeartbeat(void) {
    ...
    @try {
        ...
        if (!cont) {
            ...
            { id l = ((id(*)(id,SEL))objc_msgSend)(cont, sel_registerName("layer")); if (l) ct = ((CGAffineTransform(*)(id,SEL))objc_msgSend)(l, sel_registerName("affineTransform")); }
            ...
            if (gCBRAnimTicks <= 40) {   // log only the first ~2s of each open (the animation window)
...
static void cbrAnimHeartbeatStart(void) {
    static int _started = 0;
    if (_started) return;   // v3.66.0: exactly one loop, ever
    _started = 1;
    cbrAnimHeartbeat();
}
...
// v4.0.8 CONTENT GATE: DBNotificationWindow persists with hidden=0 even when NO notification is visible,
// so !hidden/alpha alone kept the full-size overlay up forever = the app never scrolled (with OR without a
...
static int cbrCPViewHasText(id v, int depth) {
    ...
    @try {
        ...
        id subs = ((id(*)(id,SEL))objc_msgSend)(v, sel_registerName("subviews"));
        NSUInteger n = subs ? [subs count] : 0;
        for (NSUInteger i=0;i<n;i++){ if (cbrCPViewHasText([subs objectAtIndex:i], depth+1)) return 1; }
    } @catch(...) {}
    return 0;
}
        ...
        if (cbrGate("com.cbr.gate.notifoverlay", 1)) {
            ...
            if (_hs != 0) {
                ...
                if (nw) {
                    ...
                    shown = (!hid && al > 0.05 && cbrCPViewHasText(nw, 0)) ? 1 : 0;   // v4.0.8: only if a real notification (visible text) is present
                }
            }
