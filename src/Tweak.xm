...
%ctor {
    ...
    if (strcmp(__progname, "CarPlay") == 0) {
        ...
        { int hf = open("/var/mobile/CBR_cp_hooks.txt", O_WRONLY|O_CREAT|O_TRUNC, 0644);
          ...
          if (hf >= 0) close(hf); }
        const char msg[] = "[CBR] v3.20.98 init - v64 fill + v77/78 lock + foreground-synthesis re-trigger";
        ...
        if(_sf>=0){const char*m="[CBR-SB] v3.20.98 init - v64 fill + v77/78 lock + foreground-synthesis re-trigger";write(_sf,m,strlen(m));
